...
def main():
    ...
    model = unrolled_net(sImg, device, 10, args.dc, mc=True).to(device)
    model.load_state_dict(torch.load(args.checkpoint, map_location=device))
    model.eval()

    mask = utils.vdSampleMask(sImg, [50, 30], 0.125 * np.prod(sImg), maskType='laplace')
    ...
    with torch.no_grad():
        for idx, (kspace, sens_maps, _) in enumerate(tqdm(loader)):
            ...
            # Network output
            output = model(wx_init, mask, kspace_undersampled, sens_maps_permuted)
