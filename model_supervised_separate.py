...
class unrolled_net_separate(nn.Module):
    def __init__(self, sImg, device, linesearch=True, alpha=0.001, dc=True, wavelets=True, grad=True, n=4):
        ...
        if self.grad:
            self.grad_step = grad_desc(alpha, linesearch, w=self.W, wt=self.Wt)
...
if __name__ == "__main__":
    ...
    mask = mask > 0
    model = unrolled_net_separate([640, 320], device, linesearch=True, dc=True, n=4)

    print(summary(model, [im_in.shape, mask.shape, sMaps.shape, sMaps.shape], dtypes=[torch.complex64, torch.bool, torch.complex64, torch.complex64], device="cpu"))