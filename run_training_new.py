...
def run_training(ks, sImg, sMask, sMaps, rng, samp_frac, train_frac, 
  ...
  if sc:
    ...
    if wavelets:
      model = ZS_Unrolled_Network_wavelets(sImg, device, sMaps, 10, dc=dc)
...
def main():
  ...
  """
  # Create the parser
  parser = argparse.ArgumentParser(description='Run the zero-shot unrolled network.')
  
  # Add arguments with defaults (you can replace these with your current hardcoded paths)
  parser.add_argument('--input', type=str, required=True,
                      help='Path to the data file (should be MATLAB file with k-space and sensemaps)')
